...
void main() {
  ...
  if (rotationY > 0.0f) {
    ...
    newPosition =
        vec3(newPosition.x * cos(rotationY) + newPosition.z * sin(rotationY),
             newPosition.y,
  ...
  if (rotationZ > 0.0f) {
    ...
    newPosition =
        vec3(newPosition.x * cos(rotationZ) - newPosition.y * sin(rotationZ),
             newPosition.y * cos(rotationZ) + newPosition.x * sin(rotationZ),
             newPosition.z);
  }

  gl_Position = vec4(newPosition + translation,
                     2.0f); // jogar ele um pouquinho pra trás, pra ficar menor
